...
import type { Count } from "../db/models/Count.js";
...
export default class BookRepository implements IBookRepository {
  ...
  async getBooksCount(searchParams?: SeacrhParamsDTO): Promise<number> {
    let sql = await getSQL("getBooksCount.sql");
    const conditions = [];
    const params = [];
    ...
    const [rows] = await this.pool.execute<Count[]>(sql, params);
